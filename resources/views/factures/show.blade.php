...
@section('title', 'Facture FAC-' . str_pad($facture->id, 4, '0', STR_PAD_LEFT))

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('factures.index') }}" class="text-decoration-none">Factures</a></li>
    <li class="breadcrumb-item active">FAC-{{ str_pad($facture->id, 4, '0', STR_PAD_LEFT) }}</li>
@endsection
...
<div class="d-flex align-items-center justify-content-between mb-4">
    <div class="d-flex align-items-center gap-3">
        ...
        <div>
            <h4 style="font-weight:800;color:#f1f5f9;margin:0;letter-spacing:-.02em;">FAC-{{ str_pad($facture->id, 4, '0', STR_PAD_LEFT) }}</h4>
            <p style="color:#64748b;font-size:.75rem;margin:2px 0 0;">Émise le {{ $facture->dateEmission->format('d/m/Y') }}</p>
        </div>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('factures.pdf', $facture) }}" class="btn btn-outline-danger">
            ...
            <div class="card-body p-5">
                ...
                <div class="row mb-5">
                    ...
                    <div class="col-6 text-end">
                        ...
                        <p style="color:#94a3b8;font-size:.78rem;margin:2px 0;">N° FAC-{{ str_pad($facture->id, 4, '0', STR_PAD_LEFT) }}</p>
                        <p style="color:#94a3b8;font-size:.78rem;margin:2px 0;">Date : {{ $facture->dateEmission->format('d/m/Y') }}</p>
                        <p style="color:#94a3b8;font-size:.78rem;margin:2px 0;">Commande : CMD-{{ str_pad($facture->commande_id,4,'0',STR_PAD_LEFT) }}</p>
                    </div>
                </div>
                ...
                <div class="row mb-4">
                    <div class="col-6">
                        <div style="background:rgba(59,130,246,.08);border:1px solid rgba(59,130,246,.15);border-radius:12px;padding:1rem 1.2rem;">
                            ...
                            @if($facture->commande->client)
                                <div style="font-weight:700;color:#f1f5f9;font-size:.9rem;">{{ $facture->commande->client->nom }}</div>
                                <div style="color:#94a3b8;font-size:.78rem;margin-top:3px;">{{ $facture->commande->client->adresse ?? '' }}</div>
                                <div style="color:#94a3b8;font-size:.78rem;">{{ $facture->commande->client->telephone ?? '' }}</div>
                            @else
                    ...
                    <div class="col-6">
                        <div style="background:rgba(245,158,11,.08);border:1px solid rgba(245,158,11,.15);border-radius:12px;padding:1rem 1.2rem;">
                            ...
                            <div style="font-weight:700;color:#f1f5f9;font-size:.9rem;">{{ $facture->commande->fournisseur->nom }}</div>
                            <div style="color:#94a3b8;font-size:.78rem;margin-top:3px;">{{ $facture->commande->fournisseur->contact ?? '' }}</div>
                        </div>
                    </div>
                    @endif
                </div>
                    ...
                    <table class="table align-middle mb-0" style="border:1px solid rgba(255,255,255,.07);border-radius:10px;overflow:hidden;">
                        ...
                        <tbody>
                            ...
                            <tr style="border-bottom:1px solid rgba(255,255,255,.05);">
                                <td style="font-weight:600;color:#e2e8f0;padding:.85rem 1rem;">{{ $produit->nom }}</td>
                                <td style="color:#64748b;font-size:.78rem;padding:.85rem 1rem;">{{ $produit->type ?? '—' }}</td>
                                <td class="text-end" style="color:#e2e8f0;padding:.85rem 1rem;">{{ number_format($produit->pivot->prix_unitaire, 2, ',', ' ') }} DH</td>
                                <td class="text-center" style="font-weight:700;color:#f1f5f9;padding:.85rem 1rem;">{{ $produit->pivot->quantite }}</td>
                                <td class="text-end" style="font-weight:600;color:#e2e8f0;padding:.85rem 1rem;">{{ number_format($produit->pivot->quantite * $produit->pivot->prix_unitaire, 2, ',', ' ') }} DH</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                    ...
                    <div class="col-md-4">
                        <div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);border-radius:12px;overflow:hidden;">
                            <div class="d-flex justify-content-between align-items-center" style="padding:.7rem 1rem;border-bottom:1px solid rgba(255,255,255,.06);">
                                ...
                                <span style="color:#e2e8f0;font-weight:600;font-size:.8rem;">{{ number_format($facture->montant, 2, ',', ' ') }} DH</span>
                            </div>
                            <div class="d-flex justify-content-between align-items-center" style="padding:.7rem 1rem;border-bottom:1px solid rgba(255,255,255,.06);">
                                <span style="color:#94a3b8;font-size:.8rem;">TVA (20%)</span>
                                <span style="color:#e2e8f0;font-weight:600;font-size:.8rem;">{{ number_format($facture->montant * 0.20, 2, ',', ' ') }} DH</span>
                            </div>
                            <div class="d-flex justify-content-between align-items-center" style="padding:.85rem 1rem;background:rgba(16,185,129,.12);">
                                ...
                                <span style="color:#10b981;font-weight:800;font-size:1.1rem;">{{ number_format($facture->montant * 1.20, 2, ',', ' ') }} DH</span>
                            </div>
                        </div>
                    </div>
                </div>
