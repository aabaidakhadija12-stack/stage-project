...
<head>
    ...
    <title>@yield('title', 'AQUA MAB') — Gestion de Stock</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* ── Force dark on ALL Bootstrap cards ── */
        /* shadow-sm / border-0 / rounded-3 used in show/create/edit pages */
        .card.shadow-sm,
        .card.border-0 {
        ...
        /* dl/dt/dd inside cards */
        .card dt { color: var(--text-muted) !important; font-weight: 600; }
        .card dd { color: var(--text) !important; }
        /* hr inside cards */
        .card hr { border-color: var(--border); opacity: 1; }
        /* h4/h5 page titles */
        h4.fw-bold, h5.fw-bold { color: #f1f5f9; }
        /* Avatar icon circles in show pages */
        .rounded-circle.bg-warning-subtle { background: rgba(245,158,11,.15) !important; }
        .rounded-circle.bg-info-subtle    { background: rgba(6,182,212,.15) !important; }
        .rounded-circle.bg-primary-subtle { background: rgba(124,58,237,.15) !important; }
        .rounded-3.bg-primary-subtle      { background: rgba(124,58,237,.15) !important; }
        /* table-light tfoot */
        .table-light { background: rgba(255,255,255,.04) !important; }
        .table-light td, .table-light th { color: var(--text) !important; background: transparent !important; }
        /* table-dark thead */
        .table-dark { background: rgba(0,0,0,.4) !important; }
        .table-dark th { color: #e2e8f0 !important; border-color: rgba(255,255,255,.1) !important; }
        /* table-primary row */
        .table-primary { background: rgba(59,130,246,.12) !important; }
        .table-primary td { color: #e2e8f0 !important; }
        /* produit row bg-light in commandes/create */
        .produit-row.bg-light,
        .row.bg-light {
            background: rgba(255,255,255,.04) !important;
            border: 1px solid var(--border) !important;
        }
        /* facture invoice card — keep slightly lighter for readability */
        #invoice-content {
            background: rgba(255,255,255,.06) !important;
            border: 1px solid rgba(255,255,255,.1) !important;
        }
        #invoice-content .bg-light {
            background: rgba(255,255,255,.06) !important;
            border: 1px solid var(--border) !important;
        }
...
<nav id="sidebar">
    <div class="sb-brand">
        <div class="sb-icon"><img src="{{ asset('logo.png') }}?v=2" alt="AQUA MAB Logo"></div>
        <div>
    ...
    <div class="sb-nav">
        ...
        <a href="{{ route('dashboard') }}" class="sb-link accent-violet {{ request()->routeIs('dashboard') ? 'active' : '' }}">
        ...
        <a href="{{ route('produits.index') }}" class="sb-link accent-orange {{ request()->routeIs('produits.*') ? 'active' : '' }}">
        ...
        <a href="{{ route('commandes.index') }}" class="sb-link accent-cyan {{ request()->routeIs('commandes.*') ? 'active' : '' }}">
        ...
        <a href="{{ route('factures.index') }}" class="sb-link accent-emerald {{ request()->routeIs('factures.*') ? 'active' : '' }}">
            <span class="sb-icon-wrap"><i class="bi bi-receipt"></i></span>
            <span class="sb-label">Factures</span>
        </a>
        <div class="sb-section">Répertoire</div>
        <a href="{{ route('clients.index') }}" class="sb-link accent-blue {{ request()->routeIs('clients.*') ? 'active' : '' }}">
            <span class="sb-icon-wrap"><i class="bi bi-people"></i></span>
            <span class="sb-label">Clients</span>
        </a>
        <a href="{{ route('fournisseurs.index') }}" class="sb-link accent-amber {{ request()->routeIs('fournisseurs.*') ? 'active' : '' }}">
        ...
        <a href="{{ route('maintenances.index') }}" class="sb-link accent-rose {{ request()->routeIs('maintenances.*') ? 'active' : '' }}">
...
<div id="main">
    ...
    @endphp
    <div id="topbar" class="{{ $tbClass }}">
        <div class="topbar-left">
            <button class="btn btn-sm btn-outline-secondary d-md-none" id="sidebarToggle">
                <i class="bi bi-list fs-5"></i>
            </button>
            <div class="topbar-page">
                <div class="topbar-icon-wrap"><i class="bi bi-{{ $tbIcon }}"></i></div>
                <div class="topbar-text">
                    @if($tbTitle !== '')
                        <h1 class="topbar-title">{{ $tbTitle }}</h1>
                        @if($tbHasCrumb)
                        <ol class="breadcrumb topbar-crumb">{!! trim($__env->yieldContent('breadcrumb')) !!}</ol>
                        @else
                        <div class="topbar-sub">{{ $tbSub }}</div>
                        @endif
                    @elseif($tbHasCrumb)
                        <ol class="breadcrumb topbar-crumb mb-0">{!! trim($__env->yieldContent('breadcrumb')) !!}</ol>
                    @else
