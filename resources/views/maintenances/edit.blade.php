...
<div class="d-flex align-items-center justify-content-between mb-4">
    <div>
        <h4 class="fw-bold mb-1">Modifier la maintenance #{{ $maintenance->id }}</h4>
        <p class="text-muted small mb-0">{{ $maintenance->produit?->nom }}</p>
    </div>
                ...
                <form method="POST" action="{{ route('maintenances.update', $maintenance) }}">
                        ...
                        <select id="produit_id" name="produit_id"
                            ...
                            @foreach($produits as $produit)
                                <option value="{{ $produit->id }}"
                                    {{ old('produit_id', $maintenance->produit_id) == $produit->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="description" name="description" rows="3"
                                  class="form-control @error('description') is-invalid @enderror">{{ old('description', $maintenance->description) }}</textarea>
